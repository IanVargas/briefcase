 ...
 def object_to_list(value_object_to_list):
    ...
    for i in range (0 , len(value_object_to_list)):
      if value_object_to_list:
       if value_object_to_list[i].transaction_type == "Deposit":
...
def window_request_user_transaction_information(list_of_categories):
  ...
  transaction_type = []
  transaction_type.append(sg.Radio("Deposit","transaction" , enable_events=True, default=False,key = 'deposit'))
  transaction_type.append(sg.Radio("Expend", "Transaction" , enable_events=True, key = 'expend'))
  layout = [[lst],[transaction_type],
        [sg.Text("Input the ammount of the transaction to be done"),sg.Input("",key="amount_of_the_transaction")],
        [sg.Button("Aceptar")],
    ]
    # Crear la ventana
  ...
  while True:
        ...
        if event == sg.WIN_CLOSED:
            ...
            verification = verifications.money_to_be_added_input(values['amount_of_the_transaction'])
            if not verification:
              amount_of_the_transaction = float(values['amount_of_the_transaction'])
              category_of_the_transaction = values['-COMBO-']
              if values['deposit'] :
               type_of_the_transaction ="Deposit"
               
              elif values['expend']:
                type_of_the_transaction = "Expend"
              ...
              window_request_user_transaction_information(names) 
            window.close()    
            transaction_data = add_money_to_account(amount_of_the_transaction,category_of_the_transaction,type_of_the_transaction)
            return transaction_data

def add_money_to_account (amount_of_the_transaction,category_of_the_transaction,type_of_the_transaction):
